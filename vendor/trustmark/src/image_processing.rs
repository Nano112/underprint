...
impl TryFrom<ResidualImage> for DynamicImage {
    type Error = Error;

    fn try_from(
        ResidualImage(size, variant, mut array): ResidualImage,
    ) -> Result<Self, Self::Error> {
...
pub(super) fn apply_residual(input: DynamicImage, residual: DynamicImage) -> DynamicImage {
    match input {
        DynamicImage::ImageRgb8(mut input) => {
            let (width, height) = input.dimensions();
            let residual = residual
                .resize_exact(width, height, FilterType::Triangle)
                .into_rgb32f();

            for (target, delta) in input.pixels_mut().zip(residual.pixels()) {
                for channel in 0..3 {
                    let original = (f32::from(target[channel]) / 255.0) * 2.0 - 1.0;
                    let perturbation = delta[channel] * 2.0 - 1.0;
                    let applied =
                        ((f32::min(original + perturbation, 1.0) + 1.0) / 2.0).clamp(0.0, 1.0);
                    target[channel] = (applied * 255.0).round() as u8;
                }
            }
            DynamicImage::ImageRgb8(input)
        }
        input => apply_residual_reference(input, residual),
    }
}
...
mod tests {
    ...
    #[test]
    fn rgb8_residual_fast_path_matches_reference_quantization() {
        let input = RgbImage::from_fn(73, 51, |x, y| {
            image::Rgb([
                ((x * 17 + y * 3) % 256) as u8,
                ((x * 5 + y * 29) % 256) as u8,
                ((x * 11 + y * 7) % 256) as u8,
        ...
        let residual = Rgb32FImage::from_fn(19, 23, |x, y| {
            image::Rgb([
                0.5 + ((x % 5) as f32 - 2.0) / 100.0,
                0.5 + ((y % 7) as f32 - 3.0) / 100.0,
                0.5 + (((x + y) % 9) as f32 - 4.0) / 100.0,
            ])
        });
        let expected = apply_residual_reference(
            DynamicImage::ImageRgb8(input.clone()),
            DynamicImage::ImageRgb32F(residual.clone()),
        )
        .into_rgb8();
        let actual = apply_residual(
            DynamicImage::ImageRgb8(input),
            DynamicImage::ImageRgb32F(residual),
        )
        .into_rgb8();
